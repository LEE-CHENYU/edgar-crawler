#!/usr/bin/env python3
"""Watch HKEX RunPod Jupyter workers, download outputs, then delete pods.

The script is intentionally conservative:
- it waits until each shard has processed every manifest row and the worker
  process is no longer running;
- it packages each pod's `hkex_financials` directory on the pod;
- it streams the archive through the Jupyter `/files` endpoint;
- it verifies the local archive and extracted event-log row counts;
- it deletes pods only after local verification succeeds.
"""
...
def parse_pods(value: str) -> list[PodShard]:
    ...
    for chunk in value.split(","):
        pod_id, shard = chunk.strip().split(":", 1)
        pods.append(PodShard(pod_id=pod_id.strip(), shard=shard.strip()))
    return pods


def jupyter_headers() -> dict[str, str]:
    return {"User-Agent": USER_AGENT}


def wait_for_proxy(pod: PodShard, token: str, timeout_seconds: int = 300) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            if response.status_code == 200:
                return
            log(f"{pod.pod_id} proxy status {response.status_code}; retrying")
        except Exception as exc:
            log(f"{pod.pod_id} proxy check failed: {type(exc).__name__}: {exc}")
        time.sleep(10)
    raise TimeoutError(f"Jupyter proxy did not become ready for {pod.pod_id}")


def execute_code(pod: PodShard, token: str, code: str, timeout_seconds: int = 900) -> str:
    headers = jupyter_headers()
    wait_for_proxy(pod, token, timeout_seconds=120)
    kernel_id = ""
    ws = None
    try:
        response = requests.post(
            f"{pod.base_url}/api/kernels",
            params={"token": token},
            headers=headers,
            timeout=30,
        )
        ...
        while time.time() < deadline:
            ...
            if msg_type == "stream":
                ...
                return "".join(output)
        raise TimeoutError(f"Timed out executing code on {pod.pod_id}")
    finally:
        if ws is not None:
            ws.close()
        if kernel_id:
            try:
                requests.delete(
                    f"{pod.base_url}/api/kernels/{kernel_id}",
                    params={"token": token},
                    headers=headers,
                    timeout=30,
                )
...
shard = {pod.shard!r}
event_path = Path(f"hkex_financials/logs/hkex_financials_shard_{{shard}}.jsonl")
stdout_path = Path(f"hkex_financials/logs/stdout_shard_{{shard}}.log")

pgrep = subprocess.run(
    ["bash", "-lc", "pgrep -af hkex_financials_pipeline.py || true"],
    capture_output=True,
    text=True,
)
processes = [
    line for line in pgrep.stdout.splitlines()
...
facts = subprocess.run(
    ["bash", "-lc", "find hkex_financials/facts -name '*.jsonl.gz' 2>/dev/null | wc -l"],
    capture_output=True,
    text=True,
).stdout.strip()
summaries = subprocess.run(
    ["bash", "-lc", "find hkex_financials/summaries -name '*.json' 2>/dev/null | wc -l"],
    capture_output=True,
    text=True,
).stdout.strip()
du = subprocess.run(
    ["bash", "-lc", "du -sh hkex_financials 2>/dev/null | awk '{{print $1}}' || true"],
    capture_output=True,
    text=True,
).stdout.strip()
df = subprocess.run(
...
def package_remote_output(pod: PodShard, token: str) -> dict[str, Any]:
    tarball = f"hkex_financials_shard_{pod.shard}.tgz"
    code = f"""
import hashlib
import json
import subprocess
...
subprocess.run(["tar", "-czf", str(tarball), "hkex_financials"], check=True)
...
def count_local_events(dest: Path, shard: str) -> int:
    path = dest / "hkex_financials" / "logs" / f"hkex_financials_shard_{shard}.jsonl"
    if not path.exists():
...
def run(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        for pod in pods:
            try:
                ...
                log(
                    f"{pod.pod_id} shard {pod.shard}: progress={progress} "
                    f"running={status['running']} facts={status['facts']} statuses={status['statuses']}"
                )
            except Exception as exc:
                all_complete = False
        ...
        if all_complete:
            log("all shards complete; packaging and downloading")
            break
    ...
    for pod, status in zip(pods, statuses):
        ...
        if local_events != status["row_total"]:
            raise RuntimeError(
                f"event count mismatch for shard {pod.shard}: local={local_events} remote={status['row_total']}"
            )
    ...
    manifest_path = dest / "hkex_runpod_download_manifest.json"
    dest.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--dest",
        default="/Volumes/OWC Express 1M2/datasets/MARKET_FILINGS/derived/HKEX_FINANCIALS",
    )
    parser.add_argument("--download-dir", default="/tmp/hkex_runpod_downloads")
