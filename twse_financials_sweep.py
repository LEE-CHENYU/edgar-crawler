...
import twse_financials_extract as ax

RAW_ROOT = Path("/Volumes/OWC Express 1M2/datasets/MARKET_FILINGS/raw/TWSE_REPORTS")
OUT_DIR = Path("/Volumes/OWC Express 1M2/datasets/MARKET_FILINGS/derived/TWSE_FINANCIALS")
...
_FNAME = re.compile(r"^(\d{4})_(\d{3,6})_(\d{8})([A-Z]\d+)\.pdf$")
...
def build_targets(raw_root: Path, years: set[str] | None) -> list[dict]:
    ...
    for pdf in raw_root.rglob("*.pdf"):
        ...
        if not m:
            continue
        fy, code, date, form = m.group(1), m.group(2), m.group(3), m.group(4)
        if years and fy not in years:
            continue
        out.append({"ticker": code, "filing_date": date, "doc_id": pdf.stem,
                    "fiscal_year": int(fy), "pdf_path": str(pdf)})
    return out
